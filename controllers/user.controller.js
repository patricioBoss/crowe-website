...
export const createUser = async (req, res) => {
  ...
  try {
    ...
    const sent = await sendMail(
      msg,
      "Welcome to Angela Crowe Wealth Management",
      userData.email
    );
    console.log(sent);
    if (sent) {
...
export const deleteUser = async (req, res) => {
  const { userId } = req.query;
  console.log({ userId }, "from deleteUser");
  try {
    let fetchedUser = await User.findOne({ _id: userId });
...
export const attachProfileById = async (req, res, next) => {
  const { userId } = req.query;
  console.log({ userId }, "from attachProfileById");
  ...
  try {
    ...
    if (!user) {
      ...
      req.profile = user;
      console.log("attachProfileById is successful");
      next();
    }
  } catch (err) {
    return response(res, 500, "server error", err.message);
...
export const verify = async (req, res) => {
  try {
    ...
    const { _id, firstName, email } = req.profile;
    ...
    await sendMail(msg, "Verification Successful", email);
